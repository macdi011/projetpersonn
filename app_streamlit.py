...
if selected_track is not None and len(tracks) > 0:
    ...
    if album_id is not None and album_uri is not None:
        ...
        for idx in df_album_tracks.index:
            with st.container():
                col1, col2, col3, col4 = st.columns((4, 4, 2, 2))
                col11, col12 = st.columns((8, 2))
                col1.write(df_album_tracks['id'][idx])
                col2.write(df_album_tracks['name'][idx])
                col3.write(df_album_tracks['duration_ms'][idx])
                col4.write(df_album_tracks['explicit'][idx])
                if df_album_tracks['preview_url'][idx] is not None:
                    col11.write(df_album_tracks['preview_url'][idx])
                    with col12:
                        st.audio(df_album_tracks['preview_url'][idx], format="audio/mp3")
